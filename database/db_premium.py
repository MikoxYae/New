...
# Get users expiring in 5 minutes who haven't received final reminder (changed from 1 minute to 5 minutes)
    ...
    async for user in collection.find({"final_reminder_sent": {"$ne": True}}):
        ...
        try:
            ...
            if timedelta(0) < time_remaining <= timedelta(minutes=5):
                ...
                print(f"Found user {user['user_id']} expiring in {time_remaining}")
        except Exception as e:
            print(f"Error checking user {user.get('user_id')}: {e}")
